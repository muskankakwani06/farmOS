// Authentication System for FarmOS
class AuthManager {
  constructor() {
  ...
  loadUsers() {
    try {
      const users = localStorage.getItem('farmos_users');
  ...
  saveUsers() {
    try {
      localStorage.setItem('farmos_users', JSON.stringify(this.users));
    } catch (error) {
  ...
  setCurrentUser(user) {
    try {
      ...
      localStorage.setItem('farmos_current_user', JSON.stringify(userSession));
  ...
  getCurrentUser() {
    try {
      const user = localStorage.getItem('farmos_current_user');
  ...
  logout() {
    try {
      localStorage.removeItem('farmos_current_user');
